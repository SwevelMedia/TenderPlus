...
class PesertaTenderModel extends CI_Model
{
    ...
    public function getJumlahMenangKlpd($klpd,?string $tahun = null, ?string $npwp = null)
    {
        $this->db->select('COUNT(id_pemenang)');
        $this->db->from('pemenang');
    ...
    public function getdataPesertaTender($data){
        $this->db->select('paket.nilai_hps_paket');
        $this->db->select("CASE WHEN pemenang.npwp = peserta_tender.npwp THEN 'true' ELSE 'false' END AS status_pemenang");
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 6, 2) AS month');
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 1, 4) AS year');
        $this->db->from('peserta_tender');
    ...
    // {
    //     $this->db->select('peserta_tender.*, paket.nilai_hps_paket, paket.nama_tender, pemenang.nilai_hps,paket.lokasi_pekerjaan');
    //     $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 6, 2) AS month');
    //     $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 1, 4) AS year');
    //     $this->db->select("CASE
    //         WHEN pemenang.kode_tender IS NOT NULL AND pemenang.npwp = peserta_tender.npwp THEN 'menang'
    //         WHEN pemenang.kode_tender IS NULL AND paket.status_tender NOT IN ('Tender Sudah Selesai', 'Selesai') THEN 'sedang-diikuti'
    ...
    //     return $result;
    // }
        public function getDataTenderFilterForUserDashboard($npwp, $id_lpse, $tahun)
    {
        $this->db->select('peserta_tender.*, paket.nilai_hps_paket, paket.nama_tender, pemenang.nilai_hps,paket.lokasi_pekerjaan');
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 6, 2) AS month');
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 1, 4) AS year');
    ...
    // public function getDataTenderFilter($npwp, $id_lpse, $tahun)
    // {
    //     $this->db->select('peserta_tender.harga_penawaran,paket.kode_tender,paket.nilai_hps_paket,paket.lokasi_pekerjaan,paket.nama_tender, pemenang.nilai_hps');
    //     $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 6, 2) AS month');
    //     $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 1, 4) AS year');
    //     $this->db->select("CASE
    //         WHEN pemenang.kode_tender IS NOT NULL AND pemenang.npwp = peserta_tender.npwp THEN 'menang'
    ...
    //     $this->db->from('peserta_tender');
    //     $this->db->join('pemenang', 'pemenang.kode_tender = peserta_tender.kode_tender', 'left');
    //     // Subquery untuk membatasi data paket
    //     $this->db->join("(SELECT * FROM paket WHERE paket.status_tender NOT IN ('Gagal', 'Seleksi Batal', 'Tender Gagal', 'Seleksi Gagal', 'Tender Batal')) as paket", 'paket.kode_tender = peserta_tender.kode_tender', 'left');
    //     $this->db->where('peserta_tender.npwp', $npwp);
    //     $this->db->where('peserta_tender.harga_penawaran !=', 0);
    ...
    //     // $this->db->limit(20); // Ubah 100 sesuai dengan kebutuhan
    ...
    public function test($npwp)
    {
        $this->db->select('peserta_tender.harga_penawaran, paket.nilai_hps_paket, paket.nama_tender, pemenang.nilai_hps,paket.id_lpse');
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 6, 2) AS month');
        $this->db->select('SUBSTRING(paket.tanggal_pembuatan, 1, 4) AS year');
        $this->db->select("CASE
            WHEN pemenang.kode_tender IS NOT NULL AND pemenang.npwp = peserta_tender.npwp THEN 'menang'
    ...
    public function get_Peserta_Paket($npwp,$lpse,$tahun){
        $this->db->select('paket.tanggal_pembuatan, paket.nilai_hps_paket');
        $this->db->from('peserta_tender');
        $this->db->join('pemenang', 'pemenang.kode_tender = peserta_tender.kode_tender', 'left');
        $this->db->join('paket', 'paket.kode_tender = peserta_tender.kode_tender', 'left');
        $this->db->where('peserta_tender.npwp', $npwp);
        
        // Percabangan untuk menambahkan kondisi WHERE jika id_lpse tidak kosong
        if ($lpse != '') {
    ...
    public function get_menang_kalah_ikut($npwp,$lpse,$tahun) {
        $this->db->select("paket.tanggal_pembuatan,
                            CASE
                                WHEN pemenang.npwp = peserta_tender.npwp THEN 'menang'
                                WHEN pemenang.kode_tender IS NULL THEN 'sedang-diikuti'
                                ELSE 'kalah'
                            END AS status_peserta");
        $this->db->from('peserta_tender');
        $this->db->join('pemenang', 'pemenang.kode_tender = peserta_tender.kode_tender', 'left');
        ...
        // Percabangan untuk menambahkan kondisi WHERE jika $tahun tidak kosong
        if ($tahun != '') {
            $this->db->where("YEAR(paket.tanggal_pembuatan)", $tahun);
        }

        // Percabangan untuk menambahkan kondisi WHERE jika $lpse tidak kosong
        if ($lpse != '') {
            $this->db->where('paket.id_lpse', $lpse);
        }
    ...
    public function get_lpse_data()
    {
        // Lakukan query ke tabel lpse
        $query = $this->db->select('id_lpse, nama_lpse')
                          ->from('lpse')
                          ->get();
    ...
    public function getPaginatedData($npwp, $tahun, $lpse, $limit, $offset){
        $this->db->select('paket.kode_tender,paket.lokasi_pekerjaan, paket.nama_tender, paket.nilai_hps_paket, peserta_tender.harga_penawaran');
        $this->db->select("CASE
                                WHEN pemenang.npwp = peserta_tender.npwp THEN 'menang'
                                WHEN pemenang.kode_tender IS NULL THEN 'sedang-diikuti'
                                ELSE 'kalah'
        ...
        $this->db->join('paket', 'paket.kode_tender = peserta_tender.kode_tender', 'LEFT');
        $this->db->where('peserta_tender.npwp', $npwp);
        // $this->db->having('status_peserta', 'menang');

        // Tambahkan kondisi WHERE jika $tahun tidak kosong
        if ($tahun != '') {
            $this->db->where('YEAR(paket.tanggal_pembuatan)', $tahun);
        }

        // Tambahkan kondisi WHERE jika $lpse tidak kosong
        if ($lpse != '') {
            $this->db->where('paket.id_lpse', $lpse);
        }
    ...
    public function updatePaginatedData($npwp, $tahun, $lpse, $limit, $offset,$statusPeserta){
        $this->db->select('paket.kode_tender,paket.lokasi_pekerjaan, paket.nama_tender, paket.nilai_hps_paket, peserta_tender.harga_penawaran');
        $this->db->select("CASE
                                WHEN pemenang.npwp = peserta_tender.npwp THEN 'menang'
                                WHEN pemenang.kode_tender IS NULL THEN 'sedang-diikuti'
        ...
        // Tambahkan kondisi WHERE berdasarkan status peserta yang dipilih
        $this->db->group_start();
            if ($statusPeserta['sedang_diikuti'] === true) {
                $this->db->or_where('pemenang.kode_tender IS NULL');
            }
            if ($statusPeserta['menang'] === true) {
                $this->db->or_where('pemenang.npwp = peserta_tender.npwp');
            }
            if ($statusPeserta['kalah'] === true) {
                $this->db->or_where('pemenang.kode_tender IS NOT NULL AND pemenang.npwp != peserta_tender.npwp');
            }
        $this->db->group_end();
    ...
    public function countFilteredRows($npwp, $tahun, $lpse) {
        $this->db->select('peserta_tender.kode_tender'); // Select id (atau kolom lain yang unik) untuk menghindari duplikasi
        $this->db->from('peserta_tender');
        $this->db->join('pemenang', 'pemenang.kode_tender = peserta_tender.kode_tender', 'left');
        $this->db->join('paket', 'paket.kode_tender = peserta_tender.kode_tender', 'left');
        $this->db->where('peserta_tender.npwp', $npwp);
        ...
        // Batasi jumlah baris yang diambil menjadi 100
        $this->db->limit(100);

        $query = $this->db->get();
        return $query->num_rows();
    }
    public function countFilteredUpdatesRows($npwp, $tahun, $lpse,$statusPeserta) {
        $this->db->select('peserta_tender.kode_tender'); // Select id (atau kolom lain yang unik) untuk menghindari duplikasi
        $this->db->from('peserta_tender');
        $this->db->join('pemenang', 'pemenang.kode_tender = peserta_tender.kode_tender', 'left');
        $this->db->join('paket', 'paket.kode_tender = peserta_tender.kode_tender', 'left');
