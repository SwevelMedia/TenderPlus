...
use Google\Service\Contentwarehouse\AssistantVerticalsHomeautomationProtoPhysicalLocation;
...
class Competitor extends CI_Controller
{
    ...
    public function test(){
        $this->load->model('api/PesertaTenderModel');
            ...
            $data = '85.271.504.4-008.000';
            // $data = '01.929.486.7-609.000';

            $dataPesertaTender = $this->PesertaTenderModel->test($data);
            echo json_encode($dataPesertaTender);
            
        //    $this->load->view('tumbal', array('output_data' => $dataPesertaTender));
    }
    // public function filter_data() {
    //     $this->load->model('api/PesertaTenderModel');
    //         $data1 = '95.197.868.3-643.000';


    //     // Menerima data dari AJAX
    //     $id_lpse = 581;
    //     $tahun = '2022';
    //     $data = $this->PesertaTenderModel->test($data1); // Data yang sudah diterima sebelumnya

    //     // Proses filter data menggunakan PHP
    //     $filtered_data = array();
    //     foreach ($data as $item) {
    //         if ($item['id_lpse'] == $id_lpse && $item['year'] == $tahun) {
    //             $filtered_data[] = $item;
    //         }
    //     }

    //     // Mengirimkan kembali data yang difilter sebagai respons AJAX
    ...
    public function getDataChartByNPWP()
    {
        ...
        if ($data != null) {
            ...
            $dataPesertaTender = $this->PesertaTenderModel->getDataTenderFilter($data['npwp'], '', $data['tahun']);
            // var_dump($dataPesertaTender);

            if (empty($dataPesertaTender)) { // Periksa apakah $dataPesertaTender kosong
                // Jika $dataPesertaTender kosong, kirim data kosong untuk grafik
           ...
           foreach ($dataPesertaTender as $key => $value) {
                // Memeriksa apakah nilai kolom null
                if (
                    // $value['nilai_hps'] === null || 
                    $value['month'] === null || 
                    $value['nilai_hps_paket'] === null || 
                    $value['nama_tender'] === null || 
                    $value['year'] === null) {

                    continue; // Lewati data ini dan lanjutkan ke data berikutnya
                }

                // Lanjutkan proses pengolahan data
