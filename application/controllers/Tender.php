...
class Tender extends CI_Controller
{
    ...
    private function __api_key_wapanels(){  
	    return '8lzSYDJ5agGm0cSYUUZZmzHvFtZf3b';
	} 
    ...
    public function sendEmail($data)
    {
        ...
        if ($this->email->send()) {
            ...
            if ($data['wa_kirim'] == 0  && $data['kategori'] == '2') {
                $this->Tender_model->simpanKirimNotif($data['id_pengguna'], $data['count_tender']);
            } else if ($data['wa_kirim'] == 0  && $data['kategori'] == '4'){
                $this->Tender_model->simpanKirimNotifPemenang($data['id_pengguna'], $data['count_tender']);
            }
            return true;
        } else {
    ...
    public function send_wa($no_wa, $pesan){
        
        $data = [
            //API WAPANELS
            'api_key' => $this->__api_key_wapanels(),
            
            //API WAG.SWEVEL
            // 'api_key' => 'cXfTyS4qEgFbDKsgm7qJx2czhBh9dr',
        ...
        curl_setopt_array($curl, array(
            //ENDPOINT WAPANALES
            CURLOPT_URL => 'https://wa.srv10.wapanels.com/send-message',
            
            //ENDPOINT WAG.SWEVEL
            // CURLOPT_URL => 'https://wag.swevel.com/send-message',
    ...
    public function kirimTenderTerbaru()
    {
        ...
        foreach ($notifikasi as $penerima) {
            ...
            if ($jum_tender->jumlah > 0) {
                ...
                $wa_kirim = 0;
                if ($result->status) {
                    $this->Tender_model->simpanKirimNotif($penerima->id_pengguna, $jum_tender->jumlah);
                    $wa_kirim = 1;
                } else { 
                    $this->kirimTenderTerbaruByPengguna($penerima->id_pengguna);
                }
                

                $data = [
                    'id_pengguna' => $penerima->id_pengguna,
                    'name' => $penerima->nama,
                    'kategori' => '2',
                    'count_tender' => $jum_tender->jumlah,
                    'limit_tender' => $this->limit_paket,
                    'wa_kirim' => $wa_kirim,
    ...
    public function kirimTenderTerbaruByPengguna($id_pengguna)
    {
        ...
        if ($jum_tender->jumlah > 0) {
            ...
            $result = $this->send_wa($no_wa, $pesan);
    ...
    public function kirimPemenangTerbaru()
    {
        ...
        foreach ($notifikasi as $penerima) {
            ...
            if ($jum_tender->jumlah > 0) {
                ...
                $wa_kirim = 0;
                if ($result->status) {
                    $this->Tender_model->simpanKirimNotifPemenang($penerima->id_pengguna, $jum_tender->jumlah); 
                    $wa_kirim = 1;
                } else { 
                    $this->kirimPemenangTerbaruByPengguna($penerima->id_pengguna); 
                }
