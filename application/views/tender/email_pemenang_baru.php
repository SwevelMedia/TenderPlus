			...
			<td width="600">
				<table cellpadding="0" cellspacing="0" width="100%">
					...
					<?php if ($isUserTrial || $isUserPremium) : ?>
							<h4 style="margin: 5px 0 10px 0; font-weight: 400;">Halo <strong><?= $name ?></strong>,<br /> Terdapat <strong><?= $count_tender ?></strong> pemenang tender baru yang dapat Anda follow up bersama TenderPlus. <?= ($count_tender > $limit_tender) ? 'Berikut ' . $limit_tender . ' diantaranya' : '' ?></h4>
							<a href="<?= $baseurl ?>/suplier" class="btn">Lihat Semua Pemenang Tender Baru<a>
					<?php endif; ?>
					<?php if($isUserFree) : ?>
							<h4 style="margin: 5px 0 10px 0; font-weight: 400;">Total pemenang tender baru: <span style="background: #FFF2F2;border: 1px solid #B02D19;padding: 5px;font-size: 16px;"><strong><?= $count_tender ?></strong></span> pemenang</h4>
					<?php endif; ?>
					...
					<?php if ($isUserFree) : ?>
						...
						<tr>
							<td colspan="2"><a href="<?= $baseurl ?>/pembayaran?role=4" class="btn">Upgrade Akun Sekarang<a></td>
						</tr>
					<?php endif; ?>
