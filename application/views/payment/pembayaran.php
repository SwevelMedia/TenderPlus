...
<script>
    ...
    $(document).ready(function() {
        ...
        let role = <?= (!isset($_GET['role'])) ? 0 : $_GET['role'] ?>;
        if (id_paket == null && kategori == 0 && role == 0) {
    ...
    function hitung(harga, bulan) {
        let tipe = (sessionStorage.getItem('paket_pembelian') == null) ? <?= (!isset($_GET['role'])) ? 0 : $_GET['role'] ?> : sessionStorage.getItem('paket_pembelian');
        if(tipe == 0){
            tipe = <?= (!isset($this->session->user_data['kategori'])) ? 0 : $this->session->user_data['kategori'] ?>;
        }
        var harga = 120000;
        if (tipe == 4) {
            harga = 1000000;
    ...
    function getFitur() {
        let id_paket = (sessionStorage.getItem('paket_pembelian') == null) ? <?= (!isset($_GET['role'])) ? 0 : $_GET['role'] ?> : sessionStorage.getItem('paket_pembelian');
        if(id_paket == 0){
            id_paket = <?= (!isset($this->session->user_data['kategori'])) ? 0 : $this->session->user_data['kategori'] ?>;
        }
        $('#list-fitur').html('');
        drawhtml = '';
        $.ajax({
